...
| `--fix-verify` | bool | `false` | After `--fix`/`--fix-all` has edited the manifests, run the ecosystem's resolver in **dry-run mode** to confirm the new pins still resolve together. Installs nothing and writes nothing to the project. Supported for `requirements.txt` (pip), `package.json` (npm), and `go.mod` (`go list -m all`, without `-e` — that flag would report the very errors this check exists to find and exit 0 anyway); every other manifest reports **not checked** with the reason, because their toolchains resolve by writing a lockfile. The check also runs **before** any edit, so a clash the fix caused is told apart from one the manifest already had — only the former offers the revert, because rolling back real remediation repairs nothing a pre-existing clash. On a conflict the pins are **kept**: on a terminal you are offered a byte-exact revert, in CI the reverse edits are printed. Degrades to "not checked" — never to a verdict — when the tool is missing, too old, times out (3 min), is interrupted, or refuses for a reason about the machine rather than the pins (a PEP 668 Python, a network error). Needs the network and the toolchain, and runs the resolver twice (baseline plus verification), hence opt-in. Requires `--fix` or `--fix-all`; rejected under `--offline`. |
