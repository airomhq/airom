...
| `--fix` | bool | `false` | After the scan, open an **interactive advisory table** and rewrite the manifest pins you choose. `↑`/`↓` (or `j`/`k`, or the wheel) move; `enter`, `space`, or a click on the `[ Fix ]` cell applies that package; `a` applies every fixable package; `q` quits. Each package is bumped to the **highest** fixed version its advisories name — the one bump that clears all of them — and a bump that leaves the package's compatibility line is marked `(major)`. The bump is applied in **every** manifest that pins that version — a package declared in an `api/` and a `worker/` manifest is vulnerable through both — and only the version token on each declaring line changes. If one line has moved since the scan the others stay applied and the row reports `! partial`, never `fixed`. The package name is matched as a whole token, so a `langchain` fix can never land on `langchain-core`. Lockfiles, installed metadata, frozen binaries, `pom.xml`, and a manifest declaring a range rather than a pin are never rewritten: a package seen only there is listed as `— manual` with the reason, and a lockfile the bump outdated is named so you can regenerate it. Needs a terminal at least 46 columns by 13 rows (below either it refuses to draw, because a scrolled frame puts clicks on the wrong row); without one, AIROM prints what the table would have offered and points at `--fix-all`. Filesystem scans only (a container layer and a shallow clone are not a tree you would commit), and requires the CVE overlay. Runs **after** the AIBOM is emitted, so the document still describes the tree as the scan found it. See [cve.md](./cve.md#fixing-what-it-finds). |
| `--fix-all` | bool | `false` | The same plan as `--fix`, applied without the table: every fixable pin is rewritten and the result reported, followed by the packages that need a manual change and why. For CI and for terminals that cannot host the table. Mutually exclusive with `--fix`. |
| `--fix-verify` | bool | `false` | After `--fix`/`--fix-all` has edited the manifests, run the ecosystem's resolver in **dry-run mode** to confirm the new pins still resolve together. Installs nothing, and leaves the project as it found it — including putting back the bookkeeping files some tools rewrite even in dry-run mode (`npm install --dry-run` rewrites `node_modules/.package-lock.json`, which would otherwise leave npm reporting a fixed version that is not installed). Supported for `requirements.txt` (pip), `package.json` (npm), and `go.mod` (`go list -m all`, without `-e` — that flag would report the very errors this check exists to find and exit 0 anyway); every other manifest reports **not checked** with the reason, because their toolchains resolve by writing a lockfile. The check also runs **before** any edit, so a clash the fix caused is told apart from one the manifest already had — only the former offers the revert, because rolling back real remediation repairs nothing a pre-existing clash. On a conflict the pins are **kept**: on a terminal you are offered a byte-exact revert, in CI the reverse edits are printed. Degrades to "not checked" — never to a verdict — when the tool is missing, too old, times out (3 min), is interrupted, or refuses for a reason about the machine rather than the pins (a PEP 668 Python, a network error). Needs the network and the toolchain, and runs the resolver twice (baseline plus verification), hence opt-in. Requires `--fix` or `--fix-all`; rejected under `--offline`. |
| `--fix-install` | bool | `false` | After `--fix`/`--fix-all` has edited the manifests, run the project's **real package manager** so the new versions are actually resolved and installed. **This writes**: lockfiles, `node_modules/`, `go.sum`, an interpreter's `site-packages`. `package.json` → pnpm/yarn/npm chosen by the lockfile already present; `requirements.txt` → `pip install -r`; `pyproject.toml` with `uv.lock` → `uv sync`; `go.mod` → `go mod tidy`. Anything else reports **not installed** with the reason (Cargo/Poetry/Pipenv are unwired rather than guessed at, since their invocations differ across major versions). Refuses to `pip install` unless a project `.venv/` or an activated environment exists, so it can never reach the system Python. Runs the ecosystem's consistency check afterwards (`pip check`) and reports `dirty` when a tool exits `0` having left an environment whose packages disagree. Output streams live; only the last 15 lines are kept for the summary. Skipped when `--fix-verify` already found a conflict the fixes introduced. Requires `--fix` or `--fix-all`; rejected under `--offline`. See [cve.md](./cve.md#making-the-fix-real----fix-install). |
...
| `--auto-update-rules` | bool | `true` | Check [airom-rules](https://github.com/airomhq/airom-rules) for a newer signed bundle before scanning, and install it if there is one. Throttled to **at most one check per day** (recorded in `<cache>/rules/autocheck.json`), and skipped entirely under `--offline`, `--no-cached-rules`, or in a CI environment (`CI`, `GITHUB_ACTIONS`, `GITLAB_CI`, …). Only a **strictly newer** version is installed, so a rollback on the server never downgrades you. Verification is identical to an explicit `airom rules update`: an unsigned or corrupt bundle is refused, and every network failure is non-fatal: the cached bundle (or the built-in packs) simply stands. When it does update, the scan says so on stderr **and** in the document's warnings, because a changed result with no commit behind it has to be explainable. Disable with `--auto-update-rules=false` or `AIROM_AUTO_UPDATE_RULES=0`. |
...
paginated and deduped by ownerRefs), extracts every container image (including init and
