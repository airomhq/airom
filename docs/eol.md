...
For every hosted model AIROM inventoried, it looks up `(provider, model-id)` in a
curated catalog of provider retirement announcements and attaches a dated,
sourced lifecycle:
...
This overlay makes assertions about other companies' products, so the rules are
strict:
...
A scheduled vendor retirement is an **availability** fact, not a defect. Filing
it as a CycloneDX vulnerability would put a non-CVE with no CVSS into the array
that downstream triage and VEX tooling treat as security findings, and tagging
the SARIF rule with `security-severity` would inflate a Code Scanning security
dashboard with something no patch can fix. It is real, it is urgent, and it is
not a vulnerability.
