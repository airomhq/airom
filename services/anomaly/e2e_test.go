...
func TestQA_E2E(t *testing.T) {
	...
	t.Run("Proximity tripwire NYC LL144", func(t *testing.T) {
		...
		for _, a := range report.Anomalies {
			if a.Type == "proximity-hiring" && a.StatuteRef == "NYC LL144" && a.Severity == "HIGH" { found = true }
		}
		if !found { t.Errorf("Expected proximity-hiring NYC LL144 HIGH") }
	})
	...
	t.Run("Proximity tripwire FCRA/ECOA", func(t *testing.T) {
		...
		found := false
		for _, a := range report.Anomalies {
			if a.Type == "proximity-credit" && a.StatuteRef == "FCRA/ECOA" && a.Severity == "HIGH" { found = true }
		}
		if !found { t.Errorf("Expected proximity-credit FCRA/ECOA HIGH") }
	})
	...
	t.Run("Proximity tripwire HIPAA", func(t *testing.T) {
		...
		found := false
		for _, a := range report.Anomalies {
			if a.Type == "proximity-healthcare" && a.StatuteRef == "HIPAA/CA AB 3030" && a.Severity == "HIGH" { found = true }
		}
		if !found { t.Errorf("Expected proximity-healthcare HIPAA HIGH") }
	})
