...
AIROM is at **v0.2.2**: feature-complete against the 10-phase plan, architecture through a multi-agent production review, with three overlays (artifact risk, CVE, model lifecycle), compliance mapping, test-scope filtering, a reworked terminal table, and a signed rule-update channel added on top. Early software — expect rough edges, and see the deferred row below for what it deliberately does not do yet. Honest ledger:
